document.addEventListener('DOMContentLoaded', function() {
     ...
     // Load status from Firebase via PHP
     fetch('http://localhost/api/get_status.php')
     .then(response => response.json())
     .then(data => {
         const status = data.status;
         console.log('Current Status:', status);

         // Example: Disable applyFilterBtn if status is "closed"
         const applyFilterBtn = document.getElementById('applyFilterBtn');
         if (status === 'closed') {
             applyFilterBtn.disabled = true;
             applyFilterBtn.textContent = 'Shortlisting Closed';
             applyFilterBtn.classList.add('btn-secondary');
             applyFilterBtn.classList.remove('btn-primary');
         }
     })
     .catch(error => {
            ...
            .then(data => {
                ...
                data.forEach(student => {
                    ...
                    row.innerHTML = `
                        ...
                        <td><button class="btn btn-info view-profile-btn" data-enrollment="${student.enrollment_no}">View Resume</button></td>
                ...
                document.querySelectorAll('.view-profile-btn').forEach(button => {
                    button.addEventListener('click', function () {
                        ...
                        const resumeUrl = `http://localhost/api/uploads/${enrollment}.pdf`;
